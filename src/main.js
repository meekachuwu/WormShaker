...
// Config
//const config = require(['./src/config.json'], function(data){return data});
var config;
$.get('src/config.json', function(data) { config = data;});
console.log(config);
var restDrag = 0.8;
var numPoints = 8;
var smooth = true;
...
// Timeout
if (config.hint.enabled) { timeout = setTimeout(function() { document.getElementById('shake').style.display = 'block'; }, config.hint.idleTime); }
...
function updateAppearance() {
	if (stress > stressLimit) {	// Peaking
		...
		pathMouth.strokeWidth	= thickness/1.5 + Math.random()*10;
